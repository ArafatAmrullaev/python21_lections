...
# for - цикл, который работает с итерируемыми объектами, цикл заканчивает свою работу, когда он дошел до конца (до последнего элемента) в итерируемом обьекте
...
for i in string:
    ...
    string = string + "hello"
    print(string)
# отработает только 5 раз, потому что у переменной string ссылка на 71 строке менялась, а у цикла (68 стр) нет


"==============Ключевые слова в циклах================"
# break - полностью выйти из цикла
# continiue - перейти к следующей итерации
...
# При итерации dict_.keys мы получим его ключи
for key in dict_.keys():
    print(key)
# 'a', 'b', 'c', 'd'

# При итерации dict_.values мы будем получать значения словаря
...
# for key, value in dict_.keys():
# Error 
# Потому что метод keys вовзращает нам только 1 элемент, а мы распаковываем его на 2 переменные
